...
If `scapy` is installed (see `requirements.txt`), each event snapshot also gets
a `packet_summary.json` (total/broadcast/multicast/ARP/DHCP/DNS/mDNS/SSDP
counts, top source MACs/IPs) parsed from the fresh `tcpdump_capture.pcap`
(bounded to the first 50,000 packets). This is **optional** and not required
for v1 — the import is wrapped in `try/except` and a missing/corrupt/empty pcap
is skipped silently.
...
and returns `401` otherwise. `collector.auth_token` resolves as:

| `auth_token` value | Behavior |
|---|---|
| `"some-string"` | That token is required on every request. |
| `null` (default) | A random token is **auto-generated once** and persisted to `collector.token_file` (default `<output_dir>/collector.token`, mode `0600`). Read it with `sudo cat /var/log/netwatch-pi/collector.token` and put it in the Windows app's `collector.auth_token`. |
| `false` | Auth **explicitly disabled** — the server is open and trusts the LAN. A warning is printed at startup. Only run open on a network you fully control: anyone who can reach the port can read all collected logs (IPs/MACs/DNS queries/hostnames) and push data. |

If the token file cannot be created/read (e.g. read-only filesystem), the
collector degrades to open mode with a startup warning rather than refusing to
start.
...
* Bad requests return proper `4xx`; request bodies are size-limited
  (JSON ingest ≤ 8 MB, zip upload ≤ 64 MB, ≤ 5000 samples per ingest request).
...
TOKEN=$(sudo cat /var/log/netwatch-pi/collector.token)   # auto-generated by default
# add  -H "Authorization: Bearer $TOKEN"  to each call (omit only if auth_token is false)
...
> metadata. The collector requires a bearer token by default (auto-generated;
> see *Authentication*); only set `"auth_token": false` on a network you fully
> control. Before sending packet captures to anyone, review them for privacy.
